...
%   data cleaning step)
% - Function removes rows that have non empty IGNORE entry (if not removed
...
% Remove ignored records - also this should have been done previously and
% is now done here again.
dataNLME = IQMcleanRemoveIGNOREDrecords(dataNLME);
...
varNames = {'IXGDF' 'ID' 'USUBJID' 'STUDY' 'STUDYN' 'TRT' 'TRTNAME' 'TIME' 'TIMEPOS' 'NT' 'TIMEUNIT' 'YTYPE' 'NAME' 'DV' 'UNIT' 'CENS' 'MDV' 'EVID' 'AMT' 'ADM' 'ROUTE' 'RATE' 'TINF'};
...
% Exchange empty strings against "."
for k=1:length(varNames),
    x = dataNLME.(varNames{k});
    if iscell(x),
        x(strmatchIQM('',x,'exact')) = {'.'};
        dataNLME.(varNames{k}) = x;
    end
end

...
%   - M1 = Ignore missing values (set to MDV)
...
NAMES_BLLOQ_present = unique(datanew.NAME(~isnan(datanew.LLOQ)));

% Find all records that are BLOQ for all names that have LLOQ information
ixBLOQ = [];
for k=1:length(NAMES_BLLOQ_present),
    % Get indices
    ixBLOQ = [ixBLOQ(:); find(datanew.VALUE <= datanew.LLOQ & strcmp(datanew.NAME,NAMES_BLLOQ_present{k}))];
end

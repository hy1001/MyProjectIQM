function createPopPK_NONMEMprojectIQM(FLAG_IV,parameterNames,FACTOR_UNITS,data,projectPath,options,modelADVAN,paramNamesODE,paramNamesADVAN)
...
%             OMEGA_GUESS0_STRING{k} = sprintf('%1.2g  FIX',(IIVvalues0(k))^2); % Convert IIV values from STD to VAR
    ...
    for k=1:length(terms),
        ...
        for k2=1:length(block),
            ix_parameters(end+1) = strmatchIQM(block{k2},parameterNames);
        end

function [] = IQMcompareModels(projectFolders,models,output,dosing,obsTimes,options,covNames,catNames,data)
...
% [] = IQMcompareModels(projectFolders,models,output,dosing,obsTimes,options)
% [] = IQMcompareModels(projectFolders,models,output,dosing,obsTimes,options,covNames,catNames,data)
%
% [INPUT]
...
for k=1:length(models),
    moddos{k} = mergemoddosIQM(models{k},dosing);
...
for k=1:length(projectFolders),
    ...
    parfor k2=1:Nsim,
        ...
        % Simulate the model
        simres = IQMsimdosing(mexMODEL{k},dosing,obsTimes,[],paramNames,paramValuesIndiv,optionsIntegrator);    
        % Get output
